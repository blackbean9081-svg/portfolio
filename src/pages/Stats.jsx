			...
			<div className="pay2">
				...
				<div className="pay2__right">
					...
					<Func
						...
						name="일자별 통계 적재"
						desc={
							"조회마다 결제 원장을 집계하면 비싸서, 하루치 결제, 환불 통계를 미리 적재합니다.\n같은 행이 있으면 update, 없으면 insert라 재적재돼도 멱등합니다."
						}
						file="StatsService.java"
						code={CODE_LOAD}
						retro={
							"통계를 조회할 때마다 계산하면 부담이 크기 때문에, 미리 집계해 별도 테이블에 쌓아두었습니다. \n같은 날짜를 다시 집계해도 중복이 쌓이지 않도록 처리했습니다."
						}
					/>
					<Func
						rich
						...
						code={CODE_HOST}
						retro={
							"월별 매출을 매달 따로 조회하면 쿼리가 많아지기 때문에, 한 번의 쿼리로 모아서 집계하도록 했습니다."
						}
					/>
					<Func
						rich
						name="응답 DTO 변환"
						desc={
							"매출 금액을 Long으로 다뤄 int 오버플로(약 21억)를 막고,\n결제 0건일 때 평균 계산의 0 나누기를 가드합니다."
						}
