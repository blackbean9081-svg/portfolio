...
class HostSalesStatsPerfTest {
    ...
    void compareTrendQueryPerformance() {
        ...
        log.info("");
        log.info("========== 호스트 매출 추이 쿼리 성능 비교 ==========");
        log.info(String.format("반복 횟수          : %,d 회", ITERATIONS));
        log.info(String.format("개선 전 (18쿼리/회) : 총 %,.1f ms | 평균 %.3f ms/회 | 누적 쿼리 %,d 개",
                beforeMs, beforeMs / ITERATIONS, ITERATIONS * 3L * MONTHS));
        log.info(String.format("개선 후 (1쿼리/회)  : 총 %,.1f ms | 평균 %.3f ms/회 | 누적 쿼리 %,d 개",
                afterMs, afterMs / ITERATIONS, (long) ITERATIONS));
        log.info(String.format("개선 배율          : %.1f 배 빠름 (%.1f%% 단축)",
                beforeMs / afterMs, (1 - afterMs / beforeMs) * 100));
        log.info("====================================================");
        log.info("");
    }`;

const CODE_NPLUS1 = `@Slf4j
class AdminListFetchJoinPerfTest {
    ...
    private void run(String label, Supplier<List<?>> before, Supplier<List<?>> after) {
        ...
        // 5) 출력
        log.info(String.format("페이지 크기        : %d 행", PAGE_SIZE));
        log.info(String.format("반복 횟수          : %,d 회", ITERATIONS));
        log.info(String.format("개선 전 (N+1)      : %,d 쿼리/회 | 총 %,.1f ms | 평균 %.3f ms/회",
                beforeQueries, beforeMs, beforeMs / ITERATIONS));
        log.info(String.format("개선 후 (fetchJoin): %,d 쿼리/회 | 총 %,.1f ms | 평균 %.3f ms/회",
                afterQueries, afterMs, afterMs / ITERATIONS));
        log.info(String.format("쿼리 수 감소       : %,d → %,d (%.1f배)",
                beforeQueries, afterQueries, afterQueries == 0 ? 0 : (double) beforeQueries / afterQueries));
        log.info(String.format("시간 개선          : %.1f배 빠름 (%.1f%% 단축)",
