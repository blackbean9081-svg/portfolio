			...
			<div className="pay2">
				...
				<div className="pay2__right">
					...
					<Func
						rich
						name="결제 준비, 검증"
						desc={"PG 호출 전 단계에서 금액, 중복 결제, 쿠폰 유효성을 모두 검증해,\n외부 결제와 DB 상태가 어긋나는 것을 방지합니다."}
						file="PayService.java"
						code={CODE_BUILD_READY}
						retro={"외부 결제 API에 승인을 요청하기 전에, 금액, 중복 결제, 쿠폰 유효성을 먼저 검증했습니다. 검증을 통과한 요청만 외부로 보내, 잘못된 결제가 PG까지 넘어가지 않도록 했습니다."}
					/>
					<Func
						rich
						name="결제 승인 (카카오페이)"
						desc={"카카오페이 최종 승인을 멱등 처리해,\n콜백이 중복 호출돼도 결제가 한 번만 반영되게 합니다."}
						file="PayService.java"
						code={CODE_APPROVE}
						retro={"결제 승인 콜백은 네트워크 재시도나 중복 클릭으로 여러 번 들어올 수 있습니다. 이미 처리된 결제인지 먼저 확인해, 같은 요청이 여러 번 와도 결제가 한 번만 반영되도록 멱등하게 처리했습니다."}
					/>
					<Func
						rich
						name="결제 완료 후처리"
						desc={"승인이 끝나면 예약 확정, 쿠폰 사용, 포인트 차감/적립을 하나의 흐름으로 묶어 처리하며,\n카카오, 토스가 공통으로 사용합니다."}
						file="PayService.java"
						code={CODE_COMPLETE}
						retro={"결제 완료 후 예약 확정, 쿠폰 사용, 포인트 처리가 한 묶음으로 일어납니다. 카카오와 토스가 이 로직을 공통으로 쓰기 때문에, 같은 코드를 반복하지 않도록 별도 메서드로 분리해 재사용했습니다."}
					/>
				</div>
			</div>
		</section>
